...
describe("addMemberPermissions integration", () => {
  ...
  it("adds permissions with complex entity, action, and target objects", async () => {
    ...
    expect(permission.meta?.__fmdx_managed_memberId).toBe(
      member.member.memberId
    );
    ...
    expect(permission.entity).toHaveProperty(
      "__fmdx_managed_permission_entity_memberId"
    );
    expect(permission.action).toHaveProperty(
      "__fmdx_managed_permission_action_memberId"
    );
    expect(permission.target).toHaveProperty(
      "__fmdx_managed_permission_target_memberId"
    );
  });
  ...
  it("adds permissions with different member IDs", async () => {
    ...
    expect(result1.permissions[0].meta?.__fmdx_managed_memberId).toBe(
      member1.member.memberId
    );
    expect(result2.permissions[0].meta?.__fmdx_managed_memberId).toBe(
  ...
  it("adds permissions with different group IDs", async () => {
    ...
    expect(result.permissions).toHaveLength(1);
    expect(result.permissions[0].meta?.__fmdx_managed_groupId).toBe(
      "different-group"
    );
  });
