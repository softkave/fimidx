...
function getGtGroupteValue(params: {
...
function getBetweenValue(params: {
  ...
  const { value, date } = params;
  const [min, max] = value;
  const minValue = getGtGroupteValue({ value: min, date });
  ...
  partQuery.forEach((part) => {
    ...
    switch (part.op) {
      ...
      case "neq":
        ...
        break;
      case "gt": {
        const value = getGtGroupteValue({ value: part.value, date });
...
export async function getManyObjs(params: {
  ...
  const { objQuery, page, limit, tag, sort, date: inputDate } = params;
