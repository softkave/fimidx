...
import { GetRoomSubscriptionsEndpointArgs } from "@/src/definitions/room";
...
async function makeQuery(params: GetRoomSubscriptionsEndpointArgs) {
  const { hashedAuthId, roomId, roomName, appId, socketId } = params;

  const andClauses: SQL[] = [];

  if (hashedAuthId) {
    andClauses.push(eq(roomSubscriptionsTable.hashedAuthId, hashedAuthId));
  }
