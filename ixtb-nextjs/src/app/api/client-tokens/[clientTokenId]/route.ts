import { getClientTokenEndpoint } from "@/src/lib/endpoints/clientTokens/getClientTokenEndpoint";
import { updateClientTokenEndpoint } from "@/src/lib/endpoints/clientTokens/updateClientTokenEndpoint";
...
const getEndpointFn = wrapUserAuthenticated(async (req, ctx, session) => {
  return getClientTokenEndpoint({ req, ctx, session });
});
...
const patchEndpointFn = wrapUserAuthenticated(async (req, ctx, session) => {
  return updateClientTokenEndpoint({ req, ctx, session });
});
